...
- `src/pro/main/ipc/handlers/local_agent/tool_definitions.ts` contains the list of all the tools available to the Codiner local agent.
...
Finally, you will need to define how to render the custom XML tag (e.g. `<codiner-$foo-tool-name>`) inside `src/components/chat/CodinerMarkdownParser.tsx` which will typically involve creating a new React component to render the custom XML tag.
