 ...
 */
export default function codinerTagger(): Plugin {
  return {
    name: "vite-plugin-codiner-tagger",
    apply: "serve",
    enforce: "pre",
    ...
    async transform(code: string, id: string) {
      try {
        ...
        walk(ast as any, {
          enter(node: any) {
            try {
              ...
              // ── 2. Check whether the tag already has data-codiner-id ───────────────
              ...
              console.warn(
                `[codiner-tagger] Warning: Failed to process JSX node in ${id}:`,
                error,
              );
            }
          },
        ...
        console.warn(
          `[codiner-tagger] Warning: Failed to transform ${id}:`,
          error,
        );
        return null;
      }
