...
/**
 * A webpack loader that adds `data-codiner-*` attributes to JSX elements.
 */
export default function codinerTaggerLoader(this: any, code: string) {
  ...
  const transform = async () => {
    try {
          ...
          try {
            ...
            const alreadyTagged = node.attributes?.some(
              (attr: any) =>
                ...
                attr.name?.name === "data-codiner-id",
            );
            if (alreadyTagged) return;

            // Build the codiner ID
            const loc = node.loc?.start;
            if (!loc) return;
            const codinerId = `${fileRelative}:${loc.line}:${loc.column}`;

            // Inject the attributes
            if (node.name.end != null) {
              ms.appendLeft(
                node.name.end,
                ` data-codiner-id="${codinerId}" data-codiner-name="${tagName}"`,
              );
              transformCount++;
            }
          } catch (error) {
            console.warn(
              `[codiner-tagger] Warning: Failed to process JSX node in ${this.resourcePath}:`,
              error,
            );
          }
        },
      ...
      console.warn(
        `[codiner-tagger] Warning: Failed to transform ${this.resourcePath}:`,
        error,
      );
      return null;
    }
  ...
  transform()
    ...
    .catch((err) => {
      console.error(`[codiner-tagger] ERROR in ${this.resourcePath}:`, err);
