...
async function verifyReleaseAssets() {
  try {
    ...
    const owner = "Subhan-Haider";
    const repo = "Codiner_Windows";
    ...
    const response = await fetch(allReleasesUrl, {
      headers: {
        ...
        "User-Agent": "codiner-release-verifier",
      },
    });
    ...
    const expectedAssets = [
      `codiner-${normalizeVersionForPlatform(version, "rpm")}-1.x86_64.rpm`,
      `codiner-${normalizeVersionForPlatform(version, "nupkg")}-full.nupkg`,
      `codiner-${version}.Setup.exe`,
      `codiner-darwin-arm64-${version}.zip`,
      `codiner-darwin-x64-${version}.zip`,
      `codiner_${normalizeVersionForPlatform(version, "deb")}_amd64.deb`,
      "RELEASES",
    ];
