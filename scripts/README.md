...
1. `codiner-{version}-1.x86_64.rpm` (Linux RPM)
2. `codiner-{version}-full.nupkg` (Windows NuGet package)
3. `codiner-{version}.Setup.exe` (Windows installer)
4. `codiner-darwin-arm64-{version}.zip` (macOS Apple Silicon)
5. `codiner-darwin-x64-{version}.zip` (macOS Intel)
6. `codiner_{version}_amd64.deb` (Linux DEB)
