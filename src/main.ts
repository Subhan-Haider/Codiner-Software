...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("codiner", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("codiner");
}
...
async function handleDeepLinkReturn(url: string) {
  // example url: "codiner://supabase-oauth-return?token=a&refreshToken=b"
  let parsed: URL;
  try {
    parsed = new URL(url);
  } catch {
  ...
  if (parsed.protocol !== "codiner:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected codiner://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  // codiner://add-mcp-server?name=Chrome%20DevTools&config=eyJjb21tYW5kIjpudWxsLCJ0eXBlIjoic3RkaW8ifQ%3D%3D
  ...
  // codiner://add-prompt?data=<base64-encoded-json>
