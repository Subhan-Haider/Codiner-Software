...
export function HelpDialog({ isOpen, onClose }: HelpDialogProps) {
  ...
  const isCodinerProUser = settings?.providerSettings?.["auto"]?.apiKey?.value;
...
## System Information
- Codiner Version: ${debugInfo.codinerVersion}
...
`;
      ...
      const githubIssueUrl = `https://github.com/Subhan-Haider/Codiner-Software/issues/new?title=${encodedTitle}&labels=${labels}&body=${encodedBody}`;
      ...
      IpcClient.getInstance().openExternalUrl(
        "https://github.com/Subhan-Haider/Codiner-Software/issues/new",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const handleSubmitChatLogs = async () => {
    ...
    try {
      ...
      const response = await fetch(
        "https://upload-logs.codiner.online/generate-upload-url",
        {
          method: "POST",
          headers: {
...
`;
          ...
          <div className="space-y-4 overflow-y-auto flex-grow">
            ...
            <div className="border rounded-md p-3">
              ...
              <div className="text-sm bg-slate-50 dark:bg-slate-900 rounded p-2 max-h-32 overflow-y-auto">
                <p>Codiner Version: {chatLogsData.debugInfo.codinerVersion}</p>
  ...
  return (
    <Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Need help with Codiner?</DialogTitle>
        </DialogHeader>
        ...
        <div className="flex flex-col space-y-4 w-full">
          {isCodinerProUser ? (
            <div className="flex flex-col space-y-2">
              ...
              >
                <SparklesIcon className="mr-2 h-5 w-5" /> Chat with Codiner help
                bot (Pro)
              </Button>
              <p className="text-sm text-muted-foreground px-2">
                Opens an in-app help chat assistant that searches through Codiner's
                docs.
              </p>
            </div>
          ) : (
