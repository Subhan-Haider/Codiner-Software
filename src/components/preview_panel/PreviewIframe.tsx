...
const ErrorBanner = ({ error, onDismiss, onAIFix, onRestart }: ErrorBannerProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Add a little chip that says "Internal error" if source is "codiner-app" */}
      {error.source === "codiner-app" && (
        ...
        <div className="relative p-2 bg-red-100 dark:bg-red-900 rounded-sm flex gap-1 items-center">
          ...
          <span className="text-sm text-red-700 dark:text-red-200">
            ...
            {isDockerError
              ...
              : error.source === "codiner-app"
                ? "Try restarting the Codiner app or restarting your computer to see if that fixes the error."
...
export const PreviewIframe = ({ loading }: { loading: boolean }) => {
    ...
    try {
      ...
      if (result.hasStaticText && iframeRef.current?.contentWindow) {
        iframeRef.current.contentWindow.postMessage(
          {
            type: "enable-codiner-text-editing",
            data: {
  ...
  const getCurrentElementStyles = () => {
    ...
    try {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: "get-codiner-component-styles",
          data: {
  ...
  useEffect(() => {
    if (iframeRef.current?.contentWindow && isComponentSelectorInitialized) {
      iframeRef.current.contentWindow.postMessage(
        { type: "codiner-pro-mode", enabled: isProMode },
        "*",
      );
    }
  ...
  useEffect(() => {
    const handleMessage = (event: MessageEvent) => {
      ...
      if (event.data?.type === "codiner-component-selector-initialized") {
      ...
      if (event.data?.type === "codiner-text-updated") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "codiner-text-finalized") {
        handleTextUpdated(event.data);
        return;
      }

      if (event.data?.type === "codiner-component-selected") {
      ...
      if (event.data?.type === "codiner-component-deselected") {
        ...
        if (componentId) {
          ...
          if (iframeRef.current?.contentWindow) {
            iframeRef.current.contentWindow.postMessage(
              {
                type: "disable-codiner-text-editing",
                data: { componentId },
              },
              "*",
            );
      ...
      if (event.data?.type === "codiner-component-coordinates-updated") {
      ...
      if (event.data?.type === "codiner-screenshot-response") {
  ...
  const handleActivateComponentSelector = () => {
    if (iframeRef.current?.contentWindow) {
      ...
      iframeRef.current.contentWindow.postMessage(
        {
          type: newIsPicking
            ? "activate-codiner-component-selector"
            : "deactivate-codiner-component-selector",
        },
        "*",
      );
    }
...
function parseComponentSelection(data: any): ComponentSelection | null {
  if (!data || data.type !== "codiner-component-selected") {
    return null;
  }
