...
import { hasCodinerProKey, type UserSettings } from "@/lib/schemas";
...
export function ProModeSelector() {
  ...
  const toggleProEnabled = () => {
    updateSettings({
      enableCodinerPro: !settings?.enableCodinerPro,
    });
  };

  const hasProKey = settings ? hasCodinerProKey(settings) : false;
  const proModeTogglable = hasProKey && Boolean(settings?.enableCodinerPro);
  ...
  return (
    <Popover>
      <Tooltip>
        ...
        <TooltipContent>Configure Codiner Pro settings</TooltipContent>
      </Tooltip>
        ...
        <div className="space-y-4">
          <div className="space-y-1">
            <h4 className="font-medium flex items-center gap-1.5">
              <Sparkles className="h-4 w-4 text-primary" />
              <span className="text-primary font-medium">Codiner Pro</span>
            </h4>
          ...
          <div className="flex flex-col gap-5">
            <SelectorRow
              id="pro-enabled"
              label="Enable Codiner Pro"
              tooltip="Uses Codiner Pro AI credits for the main AI model and Pro modes."
              isTogglable={hasProKey}
              settingEnabled={Boolean(settings?.enableCodinerPro)}
              toggle={toggleProEnabled}
            />
            <SelectorRow
              id="web-search"
              label="Web Access"
              tooltip="Allows Codiner to access the web (e.g. search for information)"
