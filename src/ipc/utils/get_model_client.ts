...
const codinerEngineUrl = process.env.CODINER_ENGINE_URL;
...
export async function getModelClient(
  ...
  const codinerApiKey = settings.providerSettings?.auto?.apiKey?.value;
  ...
  // Handle Codiner Pro override
  if (codinerApiKey && settings.enableCodinerPro) {
    // Check if the selected provider supports Codiner Pro (has a gateway prefix) OR
    // we're using local engine.
    // IMPORTANT: some providers like OpenAI have an empty string gateway prefix,
    // so we do a nullish and not a truthy check here.
    if (providerConfig.gatewayPrefix != null || codinerEngineUrl) {
      const enableSmartFilesContext = settings.enableProSmartFilesContextMode;
      const provider = createCodinerEngine({
        apiKey: codinerApiKey,
        baseURL: codinerEngineUrl ?? "https://engine.codiner.online/v1",
      ...
      logger.info(
        `\x1b[1;97;44m Using Codiner Pro API key for model: ${model.name} \x1b[0m`,
      );

      logger.info(
        `\x1b[1;30;42m Using Codiner Pro engine: ${codinerEngineUrl ?? "<prod>"} \x1b[0m`,
      );
      ...
      logger.warn(
        `Codiner Pro enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
  ...
  if (model.provider === "auto") {
    ...
    if (settings.enableCodinerPro) {
      // 1. Check valid key
      if (!hasCodinerProKey(settings)) {
        throw new Error("Codiner Pro key is missing or invalid");
      }
      // 2. Check budget (if it exists)
      if (
        settings.codinerProBudget &&
        settings.codinerProBudget.maxBudget > 0
      ) {
        // Budget logic would go here if we tracked usage locally,
        // but currently we rely on the server to reject request.
      }
    }
