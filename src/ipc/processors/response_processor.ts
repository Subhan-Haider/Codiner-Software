...
export async function dryRunSearchReplace({
  ...
  const codinerSearchReplaceTags = getCodinerSearchReplaceTags(fullResponse);
  for (const tag of codinerSearchReplaceTags) {
...
export async function processFullResponseActions(
  ...
  const appPath = getCodinerAppPath(chatWithApp.app.path);
  ...
  try {
    // Extract all tags
    const codinerWriteTags = getCodinerWriteTags(fullResponse);
    ...
    if (codinerAddDependencyPackages.length > 0) {
      try {
        await executeAddDependency({
          packages: codinerAddDependencyPackages,
          message: message,
          appPath,
        });
      } catch (error) {
        errors.push({
          message: `Failed to add dependencies: ${codinerAddDependencyPackages.join(
            ", ",
          )}`,
          error: error,
        });
    ...
    for (const tag of codinerRenameTags) {
    ...
    for (const tag of codinerSearchReplaceTags) {
      ...
      try {
        if (!fs.existsSync(fullFilePath)) {
          // Do not show warning to user because we already attempt to do a <codiner-write> tag to fix it.
        ...
        const result = applySearchReplace(original, tag.content);
        if (!result.success || typeof result.content !== "string") {
          // Do not show warning to user because we already attempt to do a <codiner-write> and/or a subsequent <codiner-search-replace> tag to fix it.
          logger.warn(
    ...
    for (const tag of codinerWriteTags) {
      const filePath = tag.path;
    ...
    hasChanges =
      ...
      codinerAddDependencyPackages.length > 0;
    ...
    if (hasChanges) {
      ...
      if (codinerAddDependencyPackages.length > 0)
        changes.push(
          `added ${codinerAddDependencyPackages.join(", ")} package(s)`,
        );
      if (codinerExecuteSqlQueries.length > 0)
        changes.push(`executed ${codinerExecuteSqlQueries.length} SQL queries`);

      let message = chatSummary
        ? `[codiner] ${chatSummary} - ${changes.join(", ")}`
        : `[codiner] ${changes.join(", ")}`;
      ...
      if (uncommittedFiles && uncommittedFiles.length > 0) {
        ...
        try {
          commitHash = await gitCommit({
            path: appPath,
            message: message + " + extra files edited outside of Codiner",
            amend: true,
          });
          logger.log(
          ...
          logger.error(
            `Failed to commit changes outside of codiner: ${uncommittedFiles.join(
              ", ",
            )}`,
          );
    ...
    ${warnings
        .map(
          (warning) =>
            `<codiner-output type="warning" message="${warning.message}">${warning.error}</codiner-output>`,
        )
        .join("\n")}
    ${errors
        .map(
          (error) =>
            `<codiner-output type="error" message="${error.message}">${error.error}</codiner-output>`,
        )
        .join("\n")}
    `;
