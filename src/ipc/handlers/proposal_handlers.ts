...
import {
  ...
  getCodinerChatSummaryTag,
  getCodinerCommandTags,
...
async function getCodebaseTokenCount(
  ...
  const codebase = (
    await extractCodebase({
      appPath: getCodinerAppPath(appPath),
  ...
  return withLock("get-proposal:" + chatId, async () => {
    ...
    try {
      ...
      if (
        ...
        const proposalTitle = getCodinerChatSummaryTag(messageContent);

        const proposalWriteFiles = getCodinerWriteTags(messageContent);
        const proposalSearchReplaceFiles =
          getCodinerSearchReplaceTags(messageContent);
        const proposalRenameFiles = getCodinerRenameTags(messageContent);
        const proposalDeleteFiles = getCodinerDeleteTags(messageContent);
        const proposalExecuteSqlQueries = getCodinerExecuteSqlTags(messageContent);
        const packagesAdded = getCodinerAddDependencyTags(messageContent);
      ...
      if (latestAssistantMessage?.content) {
        const writeTags = getCodinerWriteTags(latestAssistantMessage.content);
        ...
        const commandTags = getCodinerCommandTags(latestAssistantMessage.content);
...
const approveProposalHandler = async (
  ...
  const chatSummary = getCodinerChatSummaryTag(messageToApprove.content);
