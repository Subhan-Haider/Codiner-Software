...
function listenToProcess({
  ...
  spawnedProcess.stdout?.on("data", async (data) => {
    ...
    // isNeon because: 1) only Neon apps (for the official Codiner templates) should
...
async function executeAppInDocker({
  ...
  const containerName = `codiner-app-${appId}`;
  ...
  const dockerfilePath = path.join(appPath, "Dockerfile.codiner");
...
`;
  ...
  const buildProcess = spawn(
    "docker",
    ["build", "-f", "Dockerfile.codiner", "-t", `codiner-app-${appId}`, "."],
    {
      cwd: appPath,
      stdio: "pipe",
    },
  ...
  const process = spawn(
    ...
    [
      ...
      `${appPath}:/app`,
      "-v",
      `codiner-pnpm-${appId}:/app/.pnpm-store`,
      "-e",
...
export function registerAppHandlers() {
  ...
  handle(
    ...
    async (_, params: CopyAppParams): Promise<{ app: any }> => {
      ...
      const originalAppPath = getCodinerAppPath(originalApp.path);
      const newAppPath = getCodinerAppPath(newAppName);
  ...
  ipcMain.handle("list-apps", async () => {
    ...
    const appsWithResolvedPath = allApps.map((app) => ({
      ...app,
      resolvedPath: getCodinerAppPath(app.path),
    }));
    return {
      ...
      return withLock(appId, async () => {
        try {
          ...
          if (removeNodeModules) {
            ...
            if (runtimeMode === "docker") {
              logger.log(
                `Docker mode detected for app ${appId}. Removing Docker volumes codiner-pnpm-${appId}...`,
              );
              try {
                await removeDockerVolumesForApp(appId);
                logger.log(
                  `Removed Docker volumes for app ${appId} (codiner-pnpm-${appId}).`,
                );
              } catch (e) {
  ...
  ipcMain.handle(
    ...
    async (_, params: ExportAppAsZipParams): Promise<void> => {
      ...
      if (!isWindows) {
        throw new Error(
          "Export as ZIP is currently only supported on Windows.",
        );
      }

      const tempDir = path.join(
        app.getPath("temp"),
        `codiner-export-${appId}-${Date.now()}`,
      );
      await fsPromises.mkdir(tempDir, { recursive: true });

      try {
        await copyDir(appPath, tempDir, undefined, { excludeNodeModules: true });

        // Remove .git folder from the temp directory if it exists
        const gitPath = path.join(tempDir, ".git");
        if (fs.existsSync(gitPath)) {
          await fsPromises.rm(gitPath, { recursive: true, force: true });
        }

        // Add a metadata file to the export
        const settings = readSettings();
        const metadata = {
        ...
        await fsPromises.writeFile(
          path.join(tempDir, "codiner-metadata.json"),
          JSON.stringify(metadata, null, 2),
          "utf8"
        );

        const psCommand = `Compress-Archive -Path "${tempDir}\\*" -DestinationPath "${filePath}" -Force`;
        await new Promise<void>((resolve, reject) => {
          spawn("powershell.exe", ["-Command", psCommand], { stdio: "pipe" })
            .on("close", (code) => {
              if (code === 0) resolve();
              else
                reject(
                  new Error(`Failed to create ZIP archive. Code: ${code}`),
                );
            })
            .on("error", reject);
        });
