...
import { isTurboEditsV2Enabled, hasCodinerProKey } from "../../lib/schemas";
  ...
  handle(
    ...
    async (event, req: TokenCountParams): Promise<TokenCountResult> => {
      ...
      let systemPrompt = constructSystemPrompt({
        aiRules: await readAiRules(getCodinerAppPath(chat.app.path)),
        chatMode:
      ...
      if (chat.app) {
        ...
        if (settings.enableCodinerPro && hasCodinerProKey(settings) && settings.enableProSmartFilesContextMode) {
          codebaseTokens = estimateTokens(
            files
              // It doesn't need to be the exact format but it's just to get a token estimate
              .map(
                (file) => `<codiner-file=${file.path}>${file.content}</codiner-file>`,
              )
              .join("\n\n"),
          );
        } else {
