...
import {
  ...
  getCodinerWriteTags,
  getCodinerDeleteTags,
  getCodinerRenameTags,
...
import z from "zod";
import { isSupabaseConnected, isTurboEditsV2Enabled, hasCodinerProKey } from "@/lib/schemas";
...
const TEMP_DIR = path.join(os.tmpdir(), "codiner-attachments");
...
async function processStreamChunks({
  ...
  for await (const part of fullStream) {
    ...
    if (part.type === "text-delta") {
      ...
      chunk += escapeCodinerTags(part.text);
    } else if (part.type === "tool-call") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeCodinerTags(JSON.stringify(part.input));
      chunk = `<codiner-mcp-tool-call server="${serverName}" tool="${toolName}">\n${content}\n</codiner-mcp-tool-call>\n`;
    } else if (part.type === "tool-result") {
      const { serverName, toolName } = parseMcpToolKey(part.toolName);
      const content = escapeCodinerTags(part.output);
      chunk = `<codiner-mcp-tool-result server="${serverName}" tool="${toolName}">\n${content}\n</codiner-mcp-tool-result>\n`;
    }
...
export function registerChatStreamHandlers() {
  handle("chat:stream", async (event, req: ChatStreamParams) => {
    ...
    try {
      ...
      let codinerRequestId: string | undefined;
      ...
      if (req.attachments && req.attachments.length > 0) {
        ...
        for (const [index, attachment] of req.attachments.entries()) {
          ...
          if (attachment.attachmentType === "upload-to-codebase") {
            ...
            const fileId = `CODINER_ATTACHMENT_${index}`;
            ...
            // Add instruction for AI to use codiner-write tag
            ...
            if (await isTextFile(filePath)) {
              try {
                attachmentInfo += `<codiner-text-attachment filename="${attachment.name}" type="${attachment.type}" path="${filePath}">
                </codiner-text-attachment>
                \n\n`;
              } catch (err) {
...
`;
      ...
      // Check for Codiner Pro budget (Codiner Pro users only)
      // Check for Codiner Pro budget (Codiner Pro users only)
      const isCodinerProUser = settings?.enableCodinerPro && hasCodinerProKey(settings);
      if (isCodinerProUser && settings?.codinerProBudget) {
        const { maxBudget, budgetResetAt } = settings.codinerProBudget;
        // Generate requestId early so it can be saved with the message
        codinerRequestId = uuidv4();
      }
      ...
      if (testResponse) {
        ...
        // additional files through <codiner-read> as needed.
        if (
          isSmartContextEnabled &&
          req.selectedComponents &&
          req.selectedComponents.length > 0
        ...
        const isDeepContextEnabled =
          settings.enableCodinerPro === true &&
        ...
        // For Codiner Pro + Deep Context, we set to 200 chat turns (+1)
        ...
        const aiRules = await readAiRules(getCodinerAppPath(updatedChat.app.path));
...
<codiner-write path="path/to/destination/filename.ext" description="Upload file to codebase">
CODINER_ATTACHMENT_X
</codiner-write>

Example for file with id of CODINER_ATTACHMENT_0:
<codiner-write path="src/components/Button.jsx" description="Upload file to codebase">
CODINER_ATTACHMENT_0
</codiner-write>
...
`;
        ...
        const limitedHistoryChatMessages = limitedMessageHistory.map((msg: any) => ({
          ...
          content:
            settings.selectedChatMode === "ask"
              ? removeCodinerTags(removeNonEssentialTags(msg.content))
        ...
        const simpleStreamText = async ({
          ...
          const providerOptions = getProviderOptions({
            codinerAppId: updatedChat.app.id,
            codinerRequestId,
            codinerDisableFiles,
            smartContextMode,
            files,
        ...
        if (settings.selectedChatMode === "agent") {
          ...
          const { fullStream } = await simpleStreamText({
            ...
            systemPromptOverride: constructSystemPrompt({
              aiRules: await readAiRules(getCodinerAppPath(updatedChat.app.path)),
              chatMode: "agent",
        ...
        try {
          ...
          if (
            ...
            while (
              ...
              fullResponse += `<codiner-output type="warning" message="Could not apply Turbo Edits properly for some of the files; re-generating code...">${formattedSearchReplaceIssues}</codiner-output>`;
              ...
              const fixSearchReplacePrompt =
                searchReplaceFixAttempts === 0
                  ? `There was an issue with the following \`(codiner|codiner)-search-replace\` tags. Make sure you use \`(codiner|codiner)-read\` to read the latest version of the file and then trying to do search & replace again.`
                  : `There was an issue with the following \`(codiner|codiner)-search-replace\` tags. Please fix the errors by generating the code changes using \`(codiner|codiner)-write\` tags instead.`;
...
${formattedSearchReplaceIssues}`,
          ...
          if (
            !abortController.signal.aborted &&
            settings.selectedChatMode !== "ask" &&
            hasUnclosedCodinerWrite(fullResponse)
          ) {
            ...
            while (
              ...
              logger.warn(
                `Received unclosed codiner-write tag, attempting to continue, attempt #${continuationAttempts + 1}`,
              );
          ...
          const addDependencies = getCodinerAddDependencyTags(fullResponse);
          if (
            !abortController.signal.aborted &&
            ...
            try {
              ...
              while (
                ...
                fullResponse += `<codiner-problem-report summary="${problemReport.problems.length} problems">
${problemReport.problems
                ...
                const virtualFileSystem = new AsyncVirtualFileSystem(
                  getCodinerAppPath(updatedChat.app.path),
                  {
                    fileExists: (fileName: string) => fileExists(fileName),
                    readFile: (fileName: string) => readFileWithCache(fileName),
                  },
                );
                const writeTags = getCodinerWriteTags(fullResponse);
                const renameTags = getCodinerRenameTags(fullResponse);
                const deletePaths = getCodinerDeleteTags(fullResponse);
...
[Response cancelled by user]`,
      ...
      if (!abortController.signal.aborted && fullResponse) {
        // Scrape from: <codiner-chat-summary>Renaming profile file</codiner-chat-title>
        const chatTitle = fullResponse.match(
          /<(?:codiner|codiner)-chat-summary>(.*?)<\/(?:codiner|codiner)-chat-summary>/,
        );
        if (chatTitle) {
          await db
            .update(chats as any)
  ...
  try {
    if (await isTextFile(filePath)) {
      ...
      const tagPattern = new RegExp(
        `<(?:codiner|codiner)-text-attachment filename="[^"]*" type="[^"]*" path="${escapedPath}">\\s*<\\/(?:codiner|codiner)-text-attachment>`,
        "g",
      );
...
export function removeProblemReportTags(text: string): string {
  const problemReportRegex =
    /<codiner-problem-report[^>]*>[\s\S]*?<\/codiner-problem-report>/g;
  return text.replace(problemReportRegex, "").trim();
}

export function removeCodinerTags(text: string): string {
  const codinerRegex = /<(?:codiner|codiner)-[^>]*>[\s\S]*?<\/(?:codiner|codiner)-[^>]*>/g;
  return text.replace(codinerRegex, "").trim();
}

export function hasUnclosedCodinerWrite(text: string): boolean {
  // Find the last opening codiner-write or codiner-write tag
  const openRegex = /<(?:codiner|codiner)-write[^>]*>/g;
  let lastOpenIndex = -1;
  let match;
  ...
  const hasClosingTag = /<\/(?:codiner|codiner)-write>/.test(textAfterLastOpen);

  return !hasClosingTag;
}

function escapeCodinerTags(text: string): string {
  // Escape codiner and codiner tags in reasoning content
  // We are replacing the opening tag with a look-alike character
  // to avoid issues where thinking content includes tags
  // and are mishandled by:
  // 1. FE markdown parser
  // 2. Main process response processor
  return text.replace(/<(?:codiner|codiner)/g, (m) => `＜${m.slice(1)}`)
    .replace(/<\/(?:codiner|codiner)/g, (m) => `＜${m.slice(1)}`);
}
