...
test("copy message content - basic functionality", async ({ po }) => {
  ...
  expect(clipboardContent).not.toContain("<codiner-");
});

test("copy message content - codiner-write conversion", async ({ po }) => {
  await po.setUp({ autoApprove: true });
  await po.importApp("minimal");
  ...
  // Should convert codiner-write to markdown format (flexible path matching)
  expect(clipboardContent).toContain("### File:");
  expect(clipboardContent).toContain("```");
  expect(clipboardContent).not.toContain("<codiner-write");
});
