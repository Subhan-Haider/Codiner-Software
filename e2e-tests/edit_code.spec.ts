...
test("edit code", async ({ po }) => {
  const editedFilePath = path.join("src", "components", "made-with-codiner.tsx");
  ...
  await po.page.getByText("made-with-codiner.tsx").click();
  await po.page
    .getByRole("code")
    .locator("div")
  ...
  await po.page
    ...
    .fill("export const MadeWithCodiner = ;");
  ...
  expect(editedFile).toContain("export const MadeWithCodiner = ;");
});
